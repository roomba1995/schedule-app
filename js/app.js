...
const App = (() => {
          ...
          <div class="card">
            ...
            <div class="card-body">
              <p style="font-size:13px;color:#7f8c8d;margin-bottom:16px">
                JSONファイルから競技・ホテル情報やスケジュールデータを読み込みます。
              </p>

              <div class="form-group">
                <label>マスターデータ (sports, hotels, venues)</label>
                <div class="upload-zone" onclick="document.getElementById('import-master').click()">
                  <div class="upload-icon">📂</div>
                  <div class="upload-text">master.json をクリックして選択</div>
                </div>
                <input type="file" id="import-master" accept=".json" class="hidden"
                  onchange="App.importMaster(this)">
              </div>

              <div class="form-group">
                <label>スケジュールデータ</label>
                <div class="upload-zone" onclick="document.getElementById('import-schedules').click()">
                  <div class="upload-icon">📂</div>
                  <div class="upload-text">schedules.json をクリックして選択</div>
                </div>
                <input type="file" id="import-schedules" accept=".json" class="hidden"
                  onchange="App.importSchedules(this)">
              </div>

              <div class="form-group">
                <label>全データ（バックアップから復元）</label>
                <div class="upload-zone" onclick="document.getElementById('import-full').click()">
                  <div class="upload-icon">📂</div>
                  <div class="upload-text">backup.json をクリックして選択</div>
                </div>
                <input type="file" id="import-full" accept=".json" class="hidden"
            ...
            <div class="card-body">
              ...
              <div style="display:grid;gap:10px;">
                <button class="btn btn-secondary" onclick="App.exportMaster()">
                  📄 マスターデータ (master.json) をダウンロード
                </button>
                <button class="btn btn-secondary" onclick="App.exportSchedules()">
                  📅 スケジュールデータ (schedules.json) をダウンロード
                </button>
                <button class="btn btn-primary" onclick="App.exportFull()">
                  💾 全データバックアップ (backup.json) をダウンロード
                </button>
  ...
  function exportMaster() {
    ExportManager.downloadJSON(DataManager.exportMasterJSON(), 'master.json');
    showToast('マスターデータをダウンロードしました');
  }

  function exportSchedules() {
    ExportManager.downloadJSON(DataManager.exportSchedulesJSON(), 'schedules.json');
    showToast('スケジュールデータをダウンロードしました');
  }

  function exportFull() {
    ExportManager.downloadJSON(DataManager.exportFullJSON(), `backup_${new Date().toISOString().slice(0,10)}.json`);
